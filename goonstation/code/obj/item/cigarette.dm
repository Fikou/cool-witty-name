...
/obj/item/clothing/mask/cigarette
	...
	New()
		...
		if (src.lit) //if we spawned lit, do something about it!
			src.lit = 0
			src.light()
	...
	afterattack(atom/target, mob/user, flag) // copied from the propuffs
		...
		else if (src.lit == 0 && isitem(target) && target:burning)
	...
	attack_self(mob/user)
		if (user.find_in_hand(src) && src.lit > 0)
	...
	proc/put_out(var/mob/user as mob, var/message as text)
		if (src.lit == 1)
			...
			playsound(get_turf(src), "sound/items/small_fire_hiss.ogg", 50, 1)
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (src.lit == 0)
			if (istype(W, /obj/item/weldingtool) && W:welding)
				...
				return
			else if ((istype(W, /obj/item/match) || istype(W, /obj/item/device/candle) || istype(W, /obj/item/clothing/mask/cigarette)) && W:lit)
			...
			if (ishuman(M))
				...
				if (H.bleeding || (H.butt_op_stage == 4 && user.zone_sel.selecting == "chest"))
					if (src.cautery_surgery(H, user, 5, src.lit))
						return
			if (M.getStatusDuration("burning") && src.lit == 0)
				if (M == user)
	...
	attack_hand(mob/user as mob)
		if (!user) return
		var/can_blow_smoke = (user.wear_mask == src && src.lit && src.reagents.total_volume > 0 && src.puff_ready)
		var/success = ( ..() )
...
/obj/item/zippo
	...
	New()
		..()
		light = new /datum/light/point
		light.set_brightness(src.light_brightness)
	...
	attack(mob/target, mob/user as mob)
		...
		user.visible_message("<span style='color:red'><b>[user]</b> waves [src] around in front of [target]'s face! OoOo, are ya scared?![src.lit ? "" : " No, probably not, since [src] is closed."]</span>")
		return
	...
	afterattack(atom/target, mob/user as mob)
		if (!lit && istype(target, /obj/reagent_dispensers/fueltank))
			...
			return
		else if (!ismob(target) && src.lit && target.reagents)
			user.show_text("You heat [target].", "blue")
	...
	suicide(var/mob/user as mob)
		if (!src.user_can_suicide(user))
			user.suiciding = 0
			return 0
		if (!src.lit) // don't need to do more than just show the message since the lighter is deleted in a moment anyway
			user.visible_message("<span style='color:red'>Without even breaking stride, [user] flips open and lights [src] in one smooth movement.</span>")
...
/obj/item/zippo/dan
	...
	icon_closed = "dan_zippo"
	icon_open = "dan_zippoon"
	light_r = 0.45
	light_g = 0.22
	light_b = 1
