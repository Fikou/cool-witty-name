...
/obj/deployable_turret
	...
	var/wait_time = 15 //wait if it can't find a target
	var/range = 7 // tiles
	var/internal_angle = 0 // used for the matrix transforms
	var/external_angle = 180 // used for determining target validity
	var/projectile_type = /datum/projectile/bullet/minigun/turret
	var/datum/projectile/current_projectile = new/datum/projectile/bullet/minigun/turret
	var/burst_size = 6 // number of shots to fire. Keep in mind the bullet's shot_count
	var/fire_rate = 8 // rate of fire in shots per second
	var/angle_arc_size = 60
