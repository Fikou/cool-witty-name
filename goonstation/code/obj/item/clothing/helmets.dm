...
/obj/item/clothing/head/helmet/bucket/hat
	...
	red
		...
		attack_self(mob/user as mob)
			boutput(usr, "<span style=\"color:blue\">You turn the bucket right side up.</span>")
			var/obj/item/reagent_containers/glass/bucket/B = new /obj/item/reagent_containers/glass/bucket(src.loc)//new /obj/item/reagent_containers/glass/bucket/red(src.loc)
			user.u_equip(src)
			user.put_in_hand_or_drop(B)
			qdel(src)
			return
