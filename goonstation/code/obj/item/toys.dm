/obj/item/toy/teddy
	...
	New()
		...
		var/ears = rand(1,7)
		var/face = rand(1,7)
		var/icon/newicon = new /icon(icon,"body")

		newicon.Blend(new /icon(icon,"face-[face]"), ICON_OVERLAY)
		newicon.Blend(new /icon(icon,"ears-[ears]"), ICON_OVERLAY)
		newicon.Blend(pick("#FFB5E2", "#FFD470", "#FFED70", "#98ED70", "#98D8FF", "#E4D8FF", "#A0FFF0", "#FFA0A0", "#BFA582"), ICON_MULTIPLY)
		newicon.Blend(new /icon(icon,"nose-[nose]"), ICON_OVERLAY)

		icon = newicon

/obj/item/toy/teddy/attack_self(mob/user as mob)
	if (ishuman(user))
		var/mob/living/carbon/human/H = user
		if (H.sims)
			H.sims.affectMotive("fun", 1)
	var/message = pick("cuddle", "fuss with", "hug", "pat", "squeeze", "playfully pummel", "gently shake")
	boutput(user, "<span style=\"color:blue\">You [message] the teddy bear.</span>")
