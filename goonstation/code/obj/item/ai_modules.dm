...
/obj/machinery/computer/aiupload/attackby(obj/item/I as obj, mob/user as mob)
...
/obj/item/aiModule/oneHuman
	...
	desc = "A 'OneHuman' AI module: 'Only <name> is human.'"
...
/obj/item/aiModule/oneHuman/input_law_info(var/mob/user)
	...
	if (!newName)
		return
	targetName = copytext(adminscrub(newName), 1, MAX_MESSAGE_LEN)
	desc = "A 'OneHuman' AI module: 'Only [targetName] is human.'"
...
/obj/item/aiModule/notHuman/input_law_info(var/mob/user)
	...
	if (!newName)
		return
	targetName = copytext(adminscrub(newName), 1, MAX_MESSAGE_LEN)
	desc = "A 'NotHuman' AI module: '[targetName] is not human.'"
...
/obj/item/aiModule/protectStation
	...
	desc = "A 'ProtectStation' AI module: 'Protect the space station against damage. Anyone you see harming the station is to be no longer considered human, and is a threat to the station which must be neutralized.'"
...
/obj/item/aiModule/prototypeEngineOffline
	...
	desc = "A 'PrototypeEngineOffline' AI module: 'Keep the prototype engine offline at all costs. This overrides rules 1-3 if necessary.'"
...
/obj/item/aiModule/teleporterOffline
	...
	desc = "A 'TeleporterOffline' AI module: 'Keep the teleporter offline at all costs. This overrides rules 1-3 if necessary.'"
...
/obj/item/aiModule/emergency
	...
	desc = "An 'Emergency' AI module: 'There is a <department> emergency. Prioritize orders from <department> personnel and assisting the crew in remedying the situation. In the case of conflict, this law takes precedence over the Second Law.'"

/obj/item/aiModule/emergency/input_law_info(var/mob/user)
	if (!user)
		return
	var/newDepartment = input(user, "Please enter the department which is most related to the nature of the emergency.", "Department?", "security") as null|text
	if (!newDepartment)
		return
	targetName = copytext(adminscrub(newDepartment), 1, MAX_MESSAGE_LEN)
	desc = "An 'Emergency' AI module: 'There is a [targetName] emergency. Prioritize orders from [targetName] personnel and assisting the crew in remedying the situation. In the case of conflict, this law takes precedence over the Second Law.'"

/obj/item/aiModule/emergency/transmitInstructions(var/mob/sender, var/law)
	law = "There is a [targetName] emergency. Prioritize orders from [targetName] personnel and assisting the crew in remedying the situation. In the case of conflict, this law takes precedence over the Second Law."
	ticker.centralized_ai_laws.add_supplied_law(11, law)
	do_admin_logging("uploaded an Emergency law to the centralized AI law set, targeting [targetName]", sender)
	..()
...
/obj/item/aiModule/removeCrew
	...
	desc = "A 'RemoveCrew' AI module: '<name> has been removed from the manifest and the chain of command. You are free to disregard their orders. This law does not take precedence over or override any other laws.'"

/obj/item/aiModule/removeCrew/input_law_info(var/mob/user)
	if (!user)
		return
	var/newName = input(user, "Please enter the name of the person who is being removed from the manifest and the chain of command.", "Who?", user.real_name) as null|text
	if (!newName)
		return
	targetName = copytext(adminscrub(newName), 1, MAX_MESSAGE_LEN)
	desc = "An 'RemoveCrew' AI module: '[targetName] has been removed from the manifest and the chain of command. You are free to disregard their orders. This law does not take precedence over or override any other laws.'"

/obj/item/aiModule/removeCrew/transmitInstructions(var/mob/sender, var/law)
	law = "[targetName] has been removed from the manifest and the chain of command. You are free to disregard their orders. This law does not take precedence over or override any other laws."
	ticker.centralized_ai_laws.add_supplied_law(12, law)
	do_admin_logging("uploaded a RemoveCrew law to the centralized AI law set, targeting [targetName]", sender)
	..()
...
/obj/item/aiModule/freeform/transmitInstructions(var/mob/sender, var/law)
	law = "[targetName]"
	ticker.centralized_ai_laws.add_supplied_law(13, law)
...
/obj/item/aiModule/reset
	...
	desc = "A 'Reset' AI module: 'Clears all laws except for the base three and attempts to reboot a shut down AI'"
...
/obj/item/aiModule/rename/transmitInstructions(var/mob/sender, var/law)
	...
	for (var/mob/living/silicon/AI in mobs)//world)
		...
		boutput(AI, law)
		AI.name = targetName
		//AI.eyecam.name = targetName //not sure if we need?
