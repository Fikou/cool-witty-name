...
/obj/item/rcd/afterattack(atom/A, mob/user as mob)
	...
	if (mode == 1)
		...
		if (istype(A, /obj/structure/girder) && !istype(A, /obj/structure/girder/displaced) && rcd_ammocheck(user, matter_create_wall_girder, 50))
			boutput(user, "Building Wall on Girder ([matter_create_wall_girder])...")
			playsound(get_turf(src), "sound/machines/click.ogg", 50, 1)
			if(do_after(user, 20))
				if (rcd_ammocheck(user, matter_create_wall_girder))
					...
					logTheThing("station", user, null, "builds a Wall on a Girder using the RCD in [user.loc.loc] ([showCoords(user.x, user.y, user.z)])")
					playsound(get_turf(src), "sound/items/Deconstruct.ogg", 50, 1)
		...
		if (istype(A, /obj/structure/girder) && rcd_ammocheck(user, matter_remove_girder, 500))
			boutput(user, "Deconstructing Girder ([matter_remove_girder])...")
			playsound(get_turf(src), "sound/machines/click.ogg", 50, 1)
			if(do_after(user, 50))
				if (rcd_ammocheck(user, matter_remove_girder))
					...
					logTheThing("station", user, null, "removes a Girder using the RCD in [user.loc.loc] ([showCoords(user.x, user.y, user.z)])")
					playsound(get_turf(src), "sound/items/Deconstruct.ogg", 50, 1)
			return

		if (istype(A, /obj/window) && rcd_ammocheck(user, matter_remove_window, 500))
			boutput(user, "Deconstructing Window ([matter_remove_window])...")
			playsound(get_turf(src), "sound/machines/click.ogg", 50, 1)
			if(do_after(user, 50))
				if (rcd_ammocheck(user, matter_remove_window))
					...
					logTheThing("station", user, null, "removes a Window using the RCD in [user.loc.loc] ([showCoords(user.x, user.y, user.z)])")
					playsound(get_turf(src), "sound/items/Deconstruct.ogg", 50, 1)
			return

		if (istype(A, /obj/lattice) && rcd_ammocheck(user, matter_remove_floor, 500))
			boutput(user, "Deconstructing Lattice ([matter_remove_floor])...")
			playsound(get_turf(src), "sound/machines/click.ogg", 50, 1)
			if(do_after(user, 50))
				if (rcd_ammocheck(user, matter_remove_floor))
					src.shitSparks()
					rcd_ammoconsume(user, matter_remove_floor)
					qdel(A)
					logTheThing("station", user, null, "removes a Lattice using the RCD in [user.loc.loc] ([showCoords(user.x, user.y, user.z)])")
					playsound(get_turf(src), "sound/items/Deconstruct.ogg", 50, 1)
			return
		...
		if (istype(A, /turf/simulated/floor) && rcd_ammocheck(user, matter_create_window, 150))
			...
			if(do_after(user, 20))
				if (rcd_ammocheck(user, matter_create_window))
					...
					logTheThing("station", user, null, "builds a Window using the RCD in [user.loc.loc] ([showCoords(user.x, user.y, user.z)])")
					playsound(get_turf(src), "sound/items/Deconstruct.ogg", 50, 1)
