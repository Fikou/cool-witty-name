...
/mob/proc/choose_name(var/retries = 3, var/forced = 0, var/forcedname = null)
	var/newname = forcedname
	for (retries, retries > 0, retries--)
		...
		if (!newname)
			...
			if (!length(newname) || length(newname <= 3) || copytext(newname,1,2) == " ")
				...
				if (forced || alert(src, "Use the name [newname]?", newname, "Yes", "No") == "Yes")
					// Update the manifest
					for (var/datum/data/record/t in data_core.general)
						if(t.fields["name"] == src.real_name)
							t.fields["name"] = newname
					// Their ID and PDA need name updates too
					if(ishuman(src))
						var/mob/living/carbon/human/H = src
						if(istype(H.slot_wear_id,/obj/item/device/pda2) && H.slot_wear_id:ID_card)
							H.slot_wear_id:ID_card.registered = newname
							H.slot_wear_id:ID_card.update_name()
						if(istype(H.slot_wear_id,/obj/item/card/id))
							H.slot_wear_id:registered = newname
							H.slot_wear_id:update_name()
						if(istype(H.slot_l_store,/obj/item/device/pda2))
							H.slot_l_store:registered = newname
							H.slot_l_store:name = "PDA-[newname]"
						if(istype(H.slot_r_store,/obj/item/device/pda2))
							H.slot_r_store:registered = newname
							H.slot_r_store:name = "PDA-[newname]"
						if(istype(H.slot_belt,/obj/item/device/pda2))
							H.slot_belt:registered = newname
							H.slot_belt:name = "PDA-[newname]"
