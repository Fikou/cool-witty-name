...
/mob/proc/sellsoul()
	if (src.mind)
		if (isdiabolical(src))
			...
			return
		else if (src.mind.sold_soul == 1)
			boutput(src, "<span style=\"color:blue\">You don't have a soul to sell!</span>")
			return
			...
			boutput(src, "<span style=\"color:blue\">Something has gone horribly wrong with your soul! Report this to the nearest coder as soon as possible!</span>")
			return
	else
		return
	...
	attackby(obj/item/W as obj, mob/user as mob)
		if (istype(W, /obj/item/pen))
			if (isdiabolical(user))
				boutput(user, "<span style=\"color:blue\">You can't sell your soul to yourself!</span>")
				return
			else if (user.mind && user.mind.sold_soul == 1)
