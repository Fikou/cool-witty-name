...
/proc/show_xp_summary(var/key, var/mob/M) //ONLY EVER SPAWN THIS
	if(xp_archive.Find(key))
		var/loadingHtml = {"<p style="color:white">Loading your XP stats. Hang on ...</p><br>"}
		...
		for(var/job in keyList)
			...
			html += {"<p style="color:white">[job] +[xpEarned]xp</p>"}
		...
		if(!hasEntries)
			html += {"<p style="color:white">No experience earned.</p><br>"}
