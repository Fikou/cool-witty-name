...
/turf/unsimulated/floor/setpieces/Azarak/lava
	...
	Entered(var/mob/living/carbon/human/M)
		...
		SPAWN_DBG(0)
			...
			SPAWN_DBG(50)
				if(M.loc == src)
					if (ishuman(M))
						M.changeStatus("paralysis", 110)
						M.changeStatus("weakened", 110)
...
/mob/living/carbon/human/npc/cultist
	...
	New()
		...
		SPAWN_DBG(0)
			...
			update_clothing()
			//src.equip_if_possible(new /obj/item/clothing/shoes/dress_shoes(src), slot_shoes)
			//src.equip_if_possible(new /obj/item/clothing/under/suit/purple(src), slot_w_uniform)
			//src.equip_if_possible(new /obj/item/clothing/suit/cultistblack/cursed(src), slot_wear_suit)
			//src.equip_if_possible(new /obj/item/clothing/mask/eldritchskull {cant_drop = 1; cant_other_remove = 1; cant_self_remove = 1} (src), slot_wear_mask)
			//src.equip_if_possible(new /obj/item/dagger/azarakknife {cant_drop = 1; cant_other_remove = 1; cant_self_remove = 1} (src), slot_r_hand)
			SPAWN_DBG(0)
