...
/proc/swapTheThing()
	...
	if(!corner)
		...
		return 0
	var/loaded = grabResource("misc/prefabs/prefab_ksol.dmm", preventCache = 1)
	if(corner && loaded)
		logTheThing("debug", null, null, "Starting SwapLoad (["misc/prefabs/prefab_ksol.dmm"]) at [time2text(world.timeofday)]")
		var/dmm_suite/D = new/dmm_suite()
		var/datum/loadedProperties/props = D.read_map(loaded,corner.x,corner.y,corner.z,"misc/prefabs/prefab_ksol.dmm")
		logTheThing("debug", null, null, "Finished SwapLoad (["misc/prefabs/prefab_ksol.dmm"]) at [time2text(world.timeofday)]")
