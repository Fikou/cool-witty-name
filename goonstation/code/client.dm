...
*/
	...
	SPAWN_DBG(30)
		...
		if (istype(src.mob, /mob/new_player))
			...
			if (src.byond_version < 512)
				if (alert(src, "Please update BYOND to version 512! Would you like to be taken to the download page? Make sure to download the beta and not the stable release.", "ALERT", "Yes", "No") == "Yes")
...
/*	if (ticker && ticker.mode && istype(ticker.mode, /datum/game_mode/sandbox))
	...
	src.tick_lag = CLIENTSIDE_TICK_LAG
	src.tick_lag = (winget( src, "menu.fps_chunky", "is-checked" ) == "true") ? CHUNKY_CLIENTSIDE_TICK_LAG : CLIENTSIDE_TICK_LAG
...
/client/verb/set_fps()
	set hidden = 1
	set name = "set-fps"
	src.tick_lag = (winget( src, "menu.fps_smooth", "is-checked" ) == "true") ? CLIENTSIDE_TICK_LAG : CHUNKY_CLIENTSIDE_TICK_LAG
