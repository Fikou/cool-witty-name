...
/mob/living/death(gibbed)
	...
	if (!gibbed)
		boutput(src, "<span style='font-size: 1.5em; color: blue;'><B>You have died!</B></span>")
		boutput(src, "<span style='font-size: 1.5em; color: blue;'><B>Use the 'Ghost' command to observe.</B></span>")
		boutput(src, "<span style='color: blue;'><B>The Ghost command can be found in the Commands tab on the top-right.</B></span>")
...
/mob/living/say(var/message)
	...
	if(src.z == 2 && istype(get_area(src),/area/afterlife/bar)) //check zlevel before doing istype
	...
	*/
	...
	if (src.mob_flags & SPEECH_BLOB)
		...
		for (var/mob/M in mobs)
			...
			if (M.client && (isblob(M) || M.client.holder))
				var/thisR = rendered
