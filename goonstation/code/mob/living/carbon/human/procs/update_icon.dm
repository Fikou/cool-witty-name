...
/mob/living/carbon/human/update_clothing(var/loop_blocker)
	...
	if (src.w_uniform)
		...
		if (istype(src.w_uniform,/obj/item/clothing/under/experimental))
	...
	if (src.shoes)
		...
		if (src.shoes.blood_DNA)
			...
			if (src.limbs && src.limbs.l_leg && !.)
				blood_image.icon_state = "left_shoeblood"//[!( src.lying ) ? null : "2"]"
				UpdateOverlays(blood_image, "bloody_shoes_l")
			else
				UpdateOverlays(null, "bloody_shoes_l")

			if (src.limbs && src.limbs.r_leg && !.)
				blood_image.icon_state = "right_shoeblood"//[!( src.lying ) ? null : "2"]"
...
/mob/living/carbon/human/UpdateDamage()
	...
	if (!isdead(src))
		...
		switch (h_pct)
			...
			if (-100 to 0)
				h_color	= "#ffffff"
		src.maptext = "<span style='color: [h_color]; text-align: center; -dm-text-outline: 1px black; font-family: \"Tahoma\"; font-size: 8px;'>[h_pct]%</span>"
	else
		src.maptext = ""
