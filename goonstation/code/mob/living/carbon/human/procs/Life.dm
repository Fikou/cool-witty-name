...
//		blood_volume = max(blood_volume, 0)
	...
	proc/handle_decomposition()
		if (src.decomp_stage >= 4 || !isdead(src) || src.mutantrace)
			return
		var/turf/T = get_turf_loc(src)
		if (!T)
			return
		var/suspend_rot = (istype(loc, /obj/machinery/atmospherics/unary/cryo_cell) || istype(loc, /obj/morgue) || (src.reagents && src.reagents.has_reagent("formaldehyde")))
		var/env_temp = 0
	...
	proc/breathe()
		...
		// so, now you do
		if (src.is_changeling() || (src.bioHolder && src.bioHolder.HasEffect("breathless")))
			if (src.losebreath)
				src.losebreath = 0
