	...
	switch(ai_state)
		...
		if(AI_ATTACKING)	//Gonna kick your ass.
			...
			if(iscarbon(ai_target))
				...
				if(isdead(target) || distance > 7 || (!src.see_invisible && target.invisibility) || (isunconscious(target) && prob(25)))
					ai_target = null
					ai_state = AI_PASSIVE
				...
				if((target.getStatusDuration("weakened") || target.getStatusDuration("stunned") || target.getStatusDuration("paralysis")) && istype(target.wear_mask, /obj/item/clothing/mask) && distance <= 1 && prob(10) && !ai_incapacitated())
				...
				else if ((target.getStatusDuration("weakened") || target.getStatusDuration("stunned") || target.getStatusDuration("paralysis")) && target:wear_suit && distance <= 1 && prob(5) && !src.r_hand && !ai_incapacitated())
...
/mob/living/carbon/human/proc/ai_pickupweapon()
	...
	if(pickup && !src.r_hand)
		src.put_in_hand_or_drop(pickup)
	src.set_clothing_icon_dirty()
