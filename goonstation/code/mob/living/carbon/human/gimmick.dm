...
/mob/living/carbon/human/satan
	...
	New()
		...
		SPAWN_DBG(0)
			...
			src.real_name = "Satan"
			src.name = "Satan"
			src.equip_if_possible(new /obj/item/clothing/under/misc/lawyer/red/demonic(src), src.slot_w_uniform)
	...
	New()
		...
		SPAWN_DBG(0)
			...
			SPAWN_DBG(10)
				...
				src.equip_if_possible(new /obj/item/clothing/under/misc/lawyer/red/demonic(src), src.slot_w_uniform)
				src.equip_if_possible(new /obj/item/clothing/suit/labcoat/hitman/satansuit(src), slot_wear_suit)
...
/mob/living/carbon/human/jester
	...
	New()
		...
		SPAWN_DBG(10)
			...
			src.equip_if_possible(new /obj/item/clothing/under/gimmick/jester(src), src.slot_w_uniform)
			src.equip_if_possible(new /obj/item/clothing/shoes/jester(src), slot_shoes)
			src.equip_if_possible(new /obj/item/storage/backpack(src), slot_back)
			src.equip_if_possible(new /obj/item/clothing/mask/jester(src), slot_wear_mask)
			src.equip_if_possible(new /obj/item/device/radio/headset(src), slot_ears)
			src.equip_if_possible(new /obj/item/clothing/head/jester(src), slot_head)
	...
	*/
	...
	New()
		...
		SPAWN_DBG(0)
			...
			src.equip_if_possible(new /obj/item/paper/postcard/owlery(src), slot_l_hand)
			//src.equip_if_possible(new /obj/item/device/radio/headset/civilian(src), slot_ears)
...
/mob/living/carbon/human/big_yank
	...
	attack_hand(mob/M)
		...
		if(isdead(src))
			return
		if (prob(30))
			say(pick("Hey you better back off [pick(JOHN_insults)]- I got work to do!","Not while we're underway, [pick(JOHN_insults)]- you wanna get us killed?"))

	attackby(obj/item/W, mob/M)
		if (istype(W, /obj/item/paper/tug/invoice))
			boutput(M, "<span style=\"color:blue\"><b>You show [W] to [src]</b> </span>")
			SPAWN_DBG(10)
				say(pick("Listen bud, is this urgent? Go wrench down the intermix on rig three, it's over spec!","This better be important, I'm riskin the whole job talkin' to ya."))
			return
		..()
	...
	was_harmed(var/mob/M as mob, var/obj/item/weapon = 0, var/special = 0)
		if(isdead(src))
			return
		if(prob(20))
			say(pick("Oh no you don't - not today, not ever!","Nice try asshole, but I ain't dropping this load so easy!","Gonna take more than that to take out THIS Juicer!","You wanna fuck around bucko? You wanna try your luck?"))
		src.target = M
		src.ai_state = 2
		src.ai_threatened = world.timeofday
		src.ai_target = M
