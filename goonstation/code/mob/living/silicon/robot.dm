		...
		if (istype(W, /obj/item/weldingtool))
			...
			if (src.brain || src.ai_interface)
				...
				if (B.owner)
					...
					if(inafterlifebar(oldmob))
						boutput(B.owner, "<span style=\"color:blue\">You feel yourself being pull out of the bar!</span>")
						qdel(oldmob)
...
//////////////////////////
	...
	proc/pick_module()
		...
		switch(mod)
			if("Brobocop")
				src.freemodule = 0
				boutput(src, "<span style=\"color:blue\">You chose the Brobocop module.</span>")
				src.module = new /obj/item/robot_module/brobocop(src)
				src.upgrades += new /obj/item/roboupgrade/repairpack(src)
			if("Chemistry")
				src.freemodule = 0
				boutput(src, "<span style=\"color:blue\">You chose the Chemistry module. It comes with a free </span>")
				src.module = new /obj/item/robot_module/chemistry(src)
				// TODO: spectroscopic upgrade (currently just part of being a chemistry cyborg)
			if("Civilian")
				src.freemodule = 0
				boutput(src, "<span style=\"color:blue\">You chose the Civilian module. It comes with a free Efficiency Upgrade.</span>")
				src.module = new /obj/item/robot_module/civilian(src)
	...
	proc/handle_regular_status_updates()
		...
		if (!isdead(src)) //Alive.
			...
			if (src.getStatusDuration("paralysis") || src.getStatusDuration("stunned") || src.getStatusDuration("weakened")) //WTF WTF WTFFFFFFF I BET THIS WILL BREAK THINGS
				if (isalive(src)) src.lastgasp() // calling lastgasp() here because we just got knocked out
				setunconscious(src)
			else setalive(src)
