...
/mob/living/silicon/ghostdrone
	...
	proc/setFace(type = "happy", color = "#7fc5ed")
		...
		if (src.charging || src.getStatusDuration("stunned")) //Save state but don't apply changes if charging or stunned
			src.faceType = type
			src.faceColor = color
			return 1
		...
		if (src.faceType != type) // somehow didn't get set.
			src.faceType = type
		if (src.faceColor != color)
			src.faceColor = color
		light.enable()
	...
	proc/takeoffSheet()
		...
		if (!isdead(src)) //alive
			...
			if (src.charging)
				src.updateSprite()
			if (src.getStatusDuration("stunned"))
				src.setStunnedEffects()
		else //dead
			src.icon_state = "drone-dead"
		return 1
	...
	*/
	...
	emag_act(var/mob/user, var/obj/item/card/emag/E)
		setFace(pick("happy", "sad", "mad"), rgb(rand(0,255), rand(0,255), rand(0,255)))
