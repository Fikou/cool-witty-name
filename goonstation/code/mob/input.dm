mob
	...
	process_move(keys)
		...
		if (isdead(src) && !iswraith(src) && !isobserver(src))
			return
		...
		if (src.move_dir)
			...
			if (src.canmove)
				...
				if (src.buckled && !istype(src.buckled, /obj/stool/chair))
					...
					var/glide = ((32 / delay) * world.tick_lag)// * (world.tick_lag / CLIENTSIDE_TICK_LAG))

					var/spacemove = 0
					if (old_loc.throw_unlimited && !src.is_spacefaring())
						//TODO OPTIMIZE THIS!!! SHIT!!!!!!
						if (!((locate(/obj/grille) in oview(1, src)) || (locate(/turf/simulated) in oview(1, src)) || (locate(/turf/unsimulated) in oview(1, src)) ||  (locate(/obj/window) in oview(1, src)) ||(locate(/obj/lattice) in oview(1, src)) ))
							spacemove = 1

					if (!spacemove)
						if(src.no_gravity && !(locate(/turf/simulated/wall) in oview(1, src)) && !(locate(/turf/unsimulated/wall) in oview(1, src)))
							spacemove = 1
