...
/datum/suppressant/radiation
	...
	suppress_act(var/datum/pathogen/P)
		if (P.infected.reagents.has_reagent("radium", P.suppression_threshold * 2) || P.infected.reagents.has_reagent("polonium", P.suppression_threshold) || P.infected.reagents.has_reagent("uranium", P.suppression_threshold * 10) || (P.infected.getStatusDuration("radiation")/10) > P.suppression_threshold * 0.1)
			if (P.stage > 3 && prob(P.advance_speed * 2))
				P.infected.show_message("<span style=\"color:blue\">You feel better.</span>")
				P.stage--
			return 1
