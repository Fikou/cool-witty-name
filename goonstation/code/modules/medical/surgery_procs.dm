...
/obj/item/proc/defibrillate(var/mob/living/carbon/human/patient as mob, var/mob/living/user as mob, var/emagged = 0, var/faulty = 0, var/obj/item/cell/cell = null, var/suiciding = 0)
	if (!ishuman(patient))
		return 0
	...
	if (emagged || (patient.health < 0 && !faulty) || (shockcure && !faulty) || (faulty && prob(25 + suiciding)) || (suiciding && prob(44)))
		...
		playsound(user.loc, "sound/impact_sounds/Energy_Hit_3.ogg", 75, 1)
		...
		if (patient.bioHolder.HasEffect("resist_electric"))
			...
			if (cell)
				...
				patient.changeStatus("paralysis", 1 SECONDS)
