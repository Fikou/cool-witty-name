/datum/random_event/major/black_hole
	...
	event_effect(var/source,var/turf/T,var/delay,var/duration)
		...
		new /obj/anomaly/bhole_spawner(T,1800)
...
/datum/particleType/bhole_warning
	...
	Apply(var/obj/particle/par)
		if(..())
			...
			first.Scale(50)
			animate(par, transform = first, time = 150, alpha = 30)

			first.Scale(0.1 / 50)
			animate(transform = first, time = 150, alpha = 5)
			first.Reset()