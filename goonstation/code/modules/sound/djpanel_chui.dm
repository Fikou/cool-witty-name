...
client/proc/open_dj_panel()
	...
	if (!src.holder && !src.non_admin_dj)
	...
	OnClick( var/client/who, var/id )
		...
		switch(id)
			...
			if("changefreq")
				...
				return
			if("changefile")
				loaded_song = (input(who, "Upload a file:", "File Uploader - No 50MB songs!", "") as sound)
				SetVar("set_file", "[loaded_song]") //string representation for display
				return
			if ("toggledj")
