	...
	if (!destroyed)
		...
		SPAWN_DBG(10)
			...
			if(rupturing && !destroyed) // has anyone drained the tank?
				...
				src.health = 0
				src.connected_port = null // convience
				healthcheck()
				var/T = get_turf(src)
...
/obj/machinery/portable_atmospherics/canister/attack_hand(var/mob/user as mob)
	...
	if (src.has_valve)
		...
		pressure_text = "Release Pressure: <A href='?src=\ref[src];pressure_adj=-10000'>Min</A> &middot; <b><A href='?src=\ref[src];setpressure=1'>[release_pressure]</A></b> &middot; <A href='?src=\ref[src];pressure_adj=10000'>Max</A>"
	else
