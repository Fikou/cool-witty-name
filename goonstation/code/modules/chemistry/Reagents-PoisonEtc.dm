...
*/
		...
		harmful/medusa
			...
			fluid_b = 30
			transparency = 255
			depletion_rate = 0.03 // 3 units is enough to cause blindness, 5,5 units to cause extra gas to spawn, 7 for death.
			// Short sightness after 20 ticks (60s), blindness at 30 ticks (90s), reagent "duplication" at 50 ticks (150s) and final petrification on 70 ticks (210s)

			reaction_turf(var/turf/T, var/volume)
				var/list/covered = holder.covered_turf()
				for(var/turf/location in covered)
					var/datum/effects/system/medusa_gas_spread/medusa_gas = new /datum/effects/system/medusa_gas_spread
					medusa_gas.set_up(15, 5, location)
					medusa_gas.start()
			...
			on_mob_life(var/mob/M,  var/mult = 0)
				...
				switch(data+= 1)
					...
					if(2 to 19) //Pretty mild messages, slowly warning the player up that they are royally fucked.
						if(prob(20)) do_stuff(0, H)
					if(20) //Near sightness, they should quickly seek help.
						H.bioHolder.AddEffect("bad_eyesight")
					if(21 to 29) //Starting to get pretty bad now
						if(prob(30)) do_stuff(1, H)
					if(30) //Blindness, they really should find someone to help them now.
						H.bioHolder.AddEffect("blind")
					if(31 to 49) //Their movement is starting to get fucked, scary messages.
						if(prob(30)) do_stuff(2, H)
					if(50) //The reagent "duplicates" itself at this point. You can fuck yourself up again if you reach this point and also the person who is trying to save you.
						var/list/covered = holder.covered_turf()
						for(var/turf/location in covered)
							var/datum/effects/system/medusa_gas_spread/medusa_gas = new /datum/effects/system/medusa_gas_spread
							medusa_gas.set_up(15, 5, location)
							medusa_gas.start()
					if(51 to 69) //WELP, if you've reached this stage, there is probably nothing anyone can do for you.
						if(prob(30)) do_stuff(2, H)
					if(70) //Petrified to stone, game over man, game over.
			...
			proc/do_stuff(var/severity, var/mob/living/carbon/human/H)
				...
				switch(severity)
					if(0)
						H.show_text(pick_string("chemistry_reagent_messages.txt", "medusa0"), "red")
					if(1)
						switch(rand(1,6))
							if(1 to 3) //Feels bad
								H.show_text(pick_string("chemistry_reagent_messages.txt", "strychnine1"), "red")
...
/mob/living/carbon/human/proc/become_stone_statue()
	var/obj/overlay/stoneman = new /obj/overlay(get_turf(src))
	src.pixel_x = 0
	src.pixel_y = 0
	src.set_loc(stoneman)
	stoneman.name = "stone statue of [src.name]"
	stoneman.desc = "Well, at least they're probably worth more now."
	stoneman.anchored = 0
	stoneman.set_density(1)
	stoneman.layer = MOB_LAYER
	stoneman.dir = src.dir
	...
	composite.ColorTone( rgb(150,150,150) ) // grey
