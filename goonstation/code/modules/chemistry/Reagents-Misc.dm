...
// two different foaming agents is overly complicated imo - IM
		...
		cryostylane
			...
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume_passed,var/list/paramslist = 0)
				var/obj/fluid/possible_fluid = holder.my_atom
				src = null
				if (isobserver(M))
					return
				...
				if ((world.time > M.last_cubed + 5 SECONDS) && M.bioHolder)
					if ((!(istype(possible_fluid) && volume_passed < 300)) && (!M.is_cold_resistant() || ischangeling(M)) && isturf(M.loc) )
						if (silent && volume_passed < 5)
							if (prob(volume_passed+5))
			...
			reaction_turf(var/turf/target, var/volume)
				...
				if (istype(T) && T.wet) //Wire: fix for Undefined variable /turf/space/var/wet (&& T.wet)
					...
					if (!locate(/obj/decal/cleanable/oil) in T)
						...
						switch(volume)
							if (5 to 19) make_cleanable(/obj/decal/cleanable/oil/streak,T)
							if (20 to INFINITY) make_cleanable(/obj/decal/cleanable/oil,T)
					SPAWN_DBG(200)
						T.wet = 0
						T.UpdateOverlays(null, "wet_overlay")
	...
	proc/process()
		while (!disposed)
			if (get_dist(src, src.deathtarget) <= 1)
				...
				deathtarget.setStatus("stunned", max(deathtarget.getStatusDuration("stunned"), 100))
				deathtarget.setStatus("weakened", max(deathtarget.getStatusDuration("weakened"), 100))
