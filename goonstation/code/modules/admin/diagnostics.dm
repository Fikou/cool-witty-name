	...
	atmos_air/GetInfo(var/turf/theTurf, var/image/theImage)
		...
		if(istype(sim, /turf/simulated))//byondood
			...
			if(group)
				...
				theImage.desc = "O2=[group.air.oxygen]<br/>Nitrogen=[group.air.nitrogen]<br/>CO2=[group.air.carbon_dioxide]<br/>Plasma=[group.air.toxins]<br/>Temperature=[group.air.temperature]"
				var/list/borders_space = list()
				for(var/turf/spaceses in group.space_borders)
					if(get_dist(spaceses, theTurf) == 1)
						var/dir = get_dir(spaceses, theTurf)
						if((dir & (dir-1)) == 0)
