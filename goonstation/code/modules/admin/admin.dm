...
/proc/message_admins(var/text, var/asay = 0, var/irc = 0)
	var/rendered = "<span class=\"admin\"><span class=\"prefix\">[irc ? "ADMIN IRC:" : "ADMIN LOG:"]</span> <span class=\"message\">[text]</span></span>"
		...
		if("notes2")
			...
			switch(href_list["type"])
				...
				if("add")
					...
					SPAWN_DBG(20) src.viewPlayerNotes(player)
	...
	//		var/list/alljobs = get_all_jobs()
		...
		if ("c_mode_current")
			if (src.level >= LEVEL_SA)
				...
				"spy","gang","disaster","changeling","vampire","mixed","mixed_rp", "construction","conspiracy","spy_theft","battle_royale", "vampire")
		...
		if("transform")
			if(( src.level >= LEVEL_PA ) || ((src.level >= LEVEL_SA) ))
				...
				var/which = input("Transform them into what?","Transform") as null|anything in list("Monkey","Cyborg","Lizardman","Squidman","Martian","Skeleton","Flashman", "Kudzuman","Ghostdrone")
				if (!which)
					return
				. = 0
				switch(which)
