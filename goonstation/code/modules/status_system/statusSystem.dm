...
/proc/testStatus()
	SPAWN_DBG(0)
		usr.changeStatus("cloaked", 10 SECONDS)
	return
...
/obj/screen/statusEffect
	...
	New()
		...
		filters += filter(type="drop_shadow",x=0,y=0,size=2, border=2,color=rgb(0,0,0))
		..()

	proc/init(var/mob/living/carbon/C, var/datum/statusEffect/S)
...
/datum/statusEffect
	...
	simplehot //Simple heal over time.
		...
		ritual
			...
			getTooltip()
				return "Healing 0.5 damage of every type, every [tickSpacing/10] sec."
	...
	simpledot //Simple damage over time.
		...
		ritual
			...
			getTooltip()
				return "Inflicting 0.5 damage of every type, every [tickSpacing/10] sec."
	...
	simpledot/burning
		...
		onAdd(var/optional = BURNING_LV1)
			...
			if(istype(owner, /mob/living))
				...
				if (ishuman(owner))
					H = owner
			/*else //UNCOMMENT WITH NEW BYOND FOR UNIVERSAL NURNING EFFECT
	...
	stonerit
		...
		onAdd(var/optional=null)
			animate(owner, color=list(0.30,0.30,0.30,0, 0.60,0.60,0.60,0, 0.10,0.10,0.10,0, 0,0,0,1, 0,0,0,0),flags=ANIMATION_PARALLEL|ANIMATION_RELATIVE, time=30)
			return

		onRemove()
			animate(owner,color=initial(owner.color),flags=ANIMATION_PARALLEL, time=30)
			return
	...
	firerit
		...
		onRemove()
			...
			owner.UpdateName()
			animate(owner,color=initial(owner.color),time=1)
			return

		onUpdate(var/timedPassed)
			wait += timedPassed
			if(wait > 10)
				if(prob(33)) shoot_projectile_ST(owner, new/datum/projectile/bullet/flare(), pick(view(8,owner)))
				addAnim()
				wait = 0
			return

		proc/addAnim()
			animate(owner,color="#ff2200",time=3,flags=ANIMATION_PARALLEL|ANIMATION_RELATIVE)
			animate(color="#ff9900",time=3)
			animate(color="#ffff00",time=3)
			animate(color=initial(owner.color),time=1)
