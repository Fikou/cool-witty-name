...
/datum/effects/system/medusa_gas_spread
	var/number = 8
	var/cardinals = 0
	var/turf/location
	var/atom/holder
...
/datum/effects/system/medusa_gas_spread/proc/set_up(n = 5, c = 0, loca, direct)
	if(n > 20)
		n = 20
	number = n
	cardinals = c
...
/datum/effects/system/medusa_gas_spread/proc/attach(atom/atom)
	holder = atom

/datum/effects/system/medusa_gas_spread/proc/start()
	var/i = 0
	for(i=0, i<src.number, i++)
		if(src.total_smoke > 20)
			return
		SPAWN_DBG(0)
			if(holder)
				src.location = get_turf(holder)
			var/obj/effects/medusa_gas/smoke = new /obj/effects/medusa_gas(src.location)
			src.total_smoke++
			var/direction = src.direction
			if(!direction)
				if(src.cardinals)
