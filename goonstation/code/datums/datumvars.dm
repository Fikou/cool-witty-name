...
*/
		...
		if (!isnull(L) && L.len > 0 && !(name == "underlays" || name == "overlays" || name == "vars" || name == "verbs" || L.len > 500))
			...
			for (var/index = 1, index <= L.len, index++)
				if (name != "contents" && name != "screen" && name != "vis_contents" && name != "vis_locs" && !isnum(L[index]) && L["[L[index]]"])
...
/client/proc/modify_variable(datum/D, variable, set_global = 0)
	...
	switch(class)
		...
		if("text")
			var/theInput = input("Enter new text:","[variable]", D.vars[variable]) as null|message
		...
		if("num")
			var/theInput = input("Enter new number:","[variable]", D.vars[variable]) as null|num
			if(theInput == null) return
			if(set_global)
				for(var/x in world)
					if(!istype(x, D.type)) continue
		...
		if("type")
			var/theInput = input("Enter type:","[variable]",D.vars[variable]) in null|typesof(/obj,/mob,/area,/turf)
			if(theInput == null) return
			if(set_global)
				for(var/x in world)
					if(!istype(x, D.type)) continue
		...
		if("reference")
			var/theInput = input("Select reference:","[variable]", D.vars[variable]) as null|mob|obj|turf|area in world
			if(theInput == null) return
			if(set_global)
				for(var/x in world)
					if(!istype(x, D.type)) continue
		...
		if("mob reference")
			var/theInput = input("Select reference:","[variable]", D.vars[variable]) as null|mob in world
			if(theInput == null) return
			if(set_global)
				for(var/x in world)
					if(!istype(x, D.type)) continue
		...
		if("file")
			var/theInput = input("Pick file:","[variable]",D.vars[variable]) as null|file
			if(theInput == null) return
			if(set_global)
				for(var/x in world)
					if(!istype(x, D.type)) continue
		...
		if("icon")
			var/theInput = input("Pick icon:","[variable]",D.vars[variable]) as null|icon
			if(theInput == null) return
			if(set_global)
				for(var/x in world)
					if(!istype(x, D.type)) continue
		...
		if("color")
			var/theInput = input("Pick color:","[variable]",D.vars[variable]) as null|color
			if(theInput == null) return
			if(set_global)
				if(isclient(D))
					for(var/client/x)
