...
datum/controller/process/fMove
	...
	doWork()
		...
		for(var/A in spacePushList)
			...
			if (!(T.turf_flags & CAN_BE_SPACE_SAMPLE || T.throw_unlimited) || T != M.loc)
				EndSpacePush(M)
				continue
			...
			if (ismob(M))
				...
				if (T.turf_flags & CAN_BE_SPACE_SAMPLE) //todo : OPTIMIZE THIS PART
					if ((!( tmob.handcuffed) && tmob.canmove))
						var/prob_slip = 5

						if (locate(/obj/grille, oview(1, tmob)) || locate(/obj/lattice, oview(1, tmob)) )
							if (!( tmob.l_hand ))
								prob_slip -= 3
							...
							if (!( tmob.r_hand ))
								...
								prob_slip -= 1
						else if (locate(/turf/unsimulated, oview(1, tmob)) || locate(/turf/simulated, oview(1, tmob)))
							if (!( tmob.l_hand ))
								prob_slip -= 2
							else if (tmob.l_hand.w_class <= 2)
								prob_slip -= 0.5
				...
				else //todo : OPTIMIZE THIS PART
					if (locate(/turf/simulated/wall) in oview(1, M) || !locate(/turf/unsimulated/wall) in oview(1, M))
						EndSpacePush(M)
						continue
