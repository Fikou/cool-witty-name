		...
		if (!map)
			...
			for (var/x = 1, x <= world.maxx, x++)
				for (var/y = 1, y <= world.maxy, y++)
					...
					var/drawx = x*2
					var/drawy = y*2
					var/drawxend = drawx + 1
					var/drawyend = drawy + 1

					if (x == 1 || x == world.maxx || y == 1 || y == world.maxx)
						map.DrawBox(rgb(150,255,150),	drawx,drawy,drawxend,drawyend)

					else if (get_hotspot(T))
						if (T.name == "asteroid" || T.name == "cavern wall" || T.type == /turf/simulated/floor/plating/airless/asteroid)
							map.DrawBox(rgb(80,0,255),	drawx,drawy,drawxend,drawyend)
						else if (T.name == "trench floor")
							map.DrawBox(rgb(80,0,50),	drawx,drawy,drawxend,drawyend)
						else
							if (T.loc && (T.loc.type == /area/shuttle/sea_elevator || T.loc.type == /area/shuttle/sea_elevator/lower))
								map.DrawBox(rgb(255, 96, 56),	drawx,drawy,drawxend,drawyend)
							else
								map.DrawBox(rgb(170,200,120),	drawx,drawy,drawxend,drawyend)
					else
						if (T.name == "asteroid" || T.name == "cavern wall" || T.type == /turf/simulated/floor/plating/airless/asteroid)
							map.DrawBox(rgb(0,0,255),	drawx,drawy,drawxend,drawyend)
						else if (T.name == "trench floor")
							map.DrawBox(rgb(0,0,50),	drawx,drawy,drawxend,drawyend)
						else
							if (T.loc && (T.loc.type == /area/shuttle/sea_elevator || T.loc.type == /area/shuttle/sea_elevator/lower))
								map.DrawBox(rgb(255, 153, 58),	drawx,drawy,drawxend,drawyend)
							else
								map.DrawBox(rgb(120,200,120),	drawx,drawy,drawxend,drawyend)

			generate_map_html()

	proc/generate_map_html()
		src.map_html = {"<html><head><meta http-equiv=\"X-UA-Compatible\" content=\"IE=8\"/></head><body><title>Trench Map</title>\
		[src.map ? "<center><img style=\"-ms-interpolation-mode:nearest-neighbor;\" src=trenchmap.png height=600 width=600></center><br>" : null]\
		<hr>\
		Blue : Natural Structure<br>\
		Green : Unnatural Structure<br>\
		Orange : NT-Operated Trench Elevator"}
...
/obj/machinery/power/vent_capture
	...
	get_desc(dist)
		...
		if (dist <= 4) //TODO : UNITS
			. += "<br>Current Output : [last_gen] W"
			. += "<br>Total Output : [total_gen] W"
