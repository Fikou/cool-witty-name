...
/datum/game_mode/gang/declare_completion()
	...
	for(var/datum/mind/leader_mind in leaders)
		...
		text += "<br><b>Cash Pile:</b> $[leader_mind.gang.score_cash]"
	...
	onStart()
		...
		try
			...
			if (M && M.mind && M.mind.gang)
				...
				switch (M.mind.gang.gear_worn(M))
					if (1)
						duration = 13 SECONDS
					if (2)
						duration = 8 SECONDS
...
/obj/ganglocker
	...
	attack_hand(var/mob/living/carbon/human/user as mob)
		...
		user.Browse(src.HTML, "window=gang_locker;size=650x600")
		//onclose(user, "gang_locker")

	//puts the html string in the var/HTML on src
	proc/generate_HTML()
		var/dat = {"<HTML>
		<h4><a href='byond://?src=\ref[src];get_gear=1'>get gear</a></h4>
		<BR>
		<h3>You have [gang.spendable_points] points to spend!</h3>
		"}
	...
	proc/insert_item(var/obj/item/item,var/mob/user)
		...
		if (istype(item, /obj/item/spacecash))
			...
			gang.score_cash += round(S.amount/200)
			gang.spendable_points += round(S.amount/200)
		...
		//drug score
			...
			var/temp_score_drug = get_I_score_drug()
			gang.score_gun += temp_score_drug
...
/datum/gang_item/ninja/shuriken
	name = "Shuriken"
	desc = "A pouch of 6 Shuriken throwing stars."
	class2 = "weapon"
	price = 500
	// item_path = /obj/item/shuriken_pouch
...
/datum/gang_item/space/discount_csaber
	name = "Faux C-Saber"
	desc = "It's not a c-saber, it's something from the discount rack. Some kinda cooky laser stick. It doesn't look very dangerous."
	class2 = "weapon"
	price = 8000
