/datum/targetable/spell/warp
	...
	cast(mob/target)
		...
		if(O && istype(O.wear_suit, /obj/item/clothing/suit/wizrobe/necro) && istype(O.head, /obj/item/clothing/head/wizard/necro))
			playsound(holder.owner.loc, "sound/voice/wizard/WarpGrim.ogg", 50, 0, -1)
		else if(holder.owner.gender == "female")
			playsound(holder.owner.loc, "sound/voice/wizard/WarpFem.ogg", 50, 0, -1)
		else
		...
		if (isrestrictedz(O.z))
			boutput(O, "<span style=\"color:blue\">You feel guilty for trying to use that spell here.</span>")
			return
