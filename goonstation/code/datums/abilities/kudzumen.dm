...
/datum/targetable/kudzu/stealth
	...
	cast(atom/T)
		...
		if (!HK.stealthed)
			...
			animate(holder.owner, alpha=90, time=3 SECONDS)
		else
			HK.stealthed = 0
...
/obj/screen/kudzu/meter
	...
	icon_state = "viney-empty"
	name = "Nutrients Meter"
	screen_loc = "EAST,CENTER+2"
	...
	proc/update()
		cur_meter_location = round((holder.points/holder.MAX_POINTS)*11)	//length of meter
