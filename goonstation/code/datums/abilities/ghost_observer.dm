...
/datum/targetable/ghost_observer/afterlife_Bar
	name = "Afterlife Bar"
	desc = "Go to the Afterlife Bar."
	...
	cast(atom/target)
		if (holder && istype(holder.owner, /mob/dead/observer))
			...
			boutput(usr, "Oop! Something broke! Just type \"afterlife-bar\" (without the quotation marks) into the bottom bar.")

/datum/targetable/ghost_observer/ass_day_arena
	name = "Fight for a new life!"
	desc = "Go to the Ass Day Arena."
	...
	cast(atom/target, params)
		...
		if (isturf(T))
			...
			switch (effect)
				...
				if (3)
					new/obj/item/device/candle/spooky/summon(T)
				if (4)
